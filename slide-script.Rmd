...
<img style="border-radius: 80%;" src="images/abono.jpg" width="180px"/>

### Olanrewaju Ahmed Babajide

#### Principal System Analyst, Data Scientist

.fade[University of Ibadan<br>Oyo, Nigeria]

[`r icons::fontawesome("link")` https://ui.edu.ng/ABOlanrewaju](https://ui.edu.ng/ABOlanrewaju)
[`r icons::fontawesome("twitter")` @abono2000](https://twitter.com/abono2000)
[`r icons::icon_style(icons::fontawesome("github"), scale = 1)` @abono2000](https://github.com/abono2000)
...
<img style="border-radius: 80%;" src="images/prof_kay.jpeg" width="180px"/>

### Ogunyemi Olakunle Joshua

#### Data Scientist

.fade[TAG NG<br>Lagos, Nigeria]

[`r icons::fontawesome("twitter")` @olakunle4impact](https://twitter.com/olakunle4impact)
...
.w-100.lh-copy[
You can install **RStudio** via this [link](https://cran.r-project.org/), choose the appropriate installer file for your operating system, download it, and then run it to install **RStudio**.
]
]
]
...
description) in `R`. You can then later use this variable’s name to easily access the value or the object that is stored with it. You can create a variable with the help of an **assignment operator** `<-` or `=`. 
]
...
```{r}
diabetic_status <- c(TRUE, FALSE, FALSE, TRUE, FALSE, TRUE)

class(diabetic_status)
```
--

```{r}
complex_vector <- c(2+1i, 3-2i, 1+1i, 4 - 2i, 2i, 1-3i)
...
.w-100.lh-copy[
You can perform a lot of mathematical analysis with complex data type (`z`) by using some of the following functions:
]
...
`Mod(z)` - modulus of a complex number
]
...
R index starts with $1$, for example:
]
...
```
.w-100.lh-copy[
To access the first element of a vector `week_day`, you type `week_day[1]` and for second element, you typed `week_day[2]`.
]
---
...
How can we access `Monday` and `Thursday` from the vector, `week_day`?
